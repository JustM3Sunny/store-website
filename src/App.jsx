...
function App() {
  ...
  const handleHover = (isHovering) => {
    gsap.to(mouseFollower.current, {
      ...
      backgroundColor: isHovering ? '#ffffff30' : 'transparent',
      duration: 0.3
    });
  };
  ...
  return (
    <>
      <div ref={mouseFollower} className="w-4 h-4 rounded-full border-2 border-white/50 fixed top-0 left-0 pointer-events-none z-50" />
      <span
        ref={growingSpan}
      ...
      <div className="w-full relative min-h-screen"ref={headingref}>
        ...
        <div className="w-full relative z-[1] h-screen sticky top-0">
          <nav className="w-full p-8 flex justify-between z-50 fixed top-0">
            <div className="brand text-2xl font-medium hover:text-pink-200 transition-colors">VibeeStudios</div>
          ...
          <div className="container mx-auto h-full flex flex-col justify-center px-8">
            <div className="textcontainer w-full max-w-4xl">
              ...
              <p className="text-xl w-[90%] mb-12 font-light opacity-80 ">
                We combine cutting-edge technology with human-centered design to create digital solutions that drive real business results.
              </p>
      ...
      <section className="relative h-[100vh] px-8 py-28 bg-black/50 backdrop-blur-xl border-t-1">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-8">
            {['Web Development', 'UI/UX Design', 'Brand Strategy'].map((service, index) => (
              <div key={index} className="p-8 rounded-2xl bg-gradient-to-br from-black/30 to-black/10 hover:bg-black/40 transition-all group">
                <div className="text-4xl mb-4 text-pink-200">0{index+1}</div>
                <h3 className="text-3xl mb-4">{service}</h3>
                <p className="text-lg opacity-80">
                  <img className="rounded-full" src="https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTQzfHx3ZWIlMjBkZXNpZ258ZW58MHx8MHx8fDA%3D%3D" alt={service} />
      ...
      <section className="relative h-screen px-8 py-28">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-20">
            ...
            <p className="text-xl max-w-xl opacity-80">A diverse team of passionate creators dedicated to pushing digital boundaries</p>
          </div>
          <div className="grid grid-cols-4 gap-6">
            {[1,2,3,4].map((_, index) => (
              <div key={index} className="aspect-square  rounded-2xl overflow-hidden group relative">
                <img
                  src={[
    ...
    <div className="text-center mt-12">
      ...
      <section className="relative h-screen px-8 py-28 bg-black/50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto flex gap-12">
          <div className="w-1/2">
            <h2 className="text-6xl mb-8">Let&apos;s Create<br />Something Amazing</h2>
            <p className="text-xl opacity-80 mb-12">We&apos;re always excited to hear about new projects and creative ideas. Let&apos;s start a conversation!</p>
          ...
          <form className="w-1/2 space-y-6">
            <input type="text" placeholder="Name" className="w-full bg-transparent border-b border-white/20 py-3 focus:border-pink-200 outline-none" />
            <input type="email" placeholder="Email" className="w-full bg-transparent border-b border-white/20 py-3 focus:border-pink-200 outline-none" />
            <textarea rows="5" placeholder="Message" className="w-full bg-transparent border-b border-white/20 py-3 focus:border-pink-200 outline-none" />
            <button className="px-4 py-2 bg-pink-200 rounded-full hover:bg-pink-300 transition-colors flex items-center justify-center">
              Send Message
              <img className="ml-2 rounded-full" src="https://images.unsplash.com/photo-1479920252409-6e3d8e8d4866?w=200&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8ODN8fHdlYiUyMGRlc2lnbnxlbnwwfHwwfHx8MA%3D%3D" alt="Send" />
            </button>
          </form>
        </div>
      </section >
      <p className="text-center text-white opacity-50 mt-8">Copyright © 2024 shanniii</p>
    </>
  );
}
